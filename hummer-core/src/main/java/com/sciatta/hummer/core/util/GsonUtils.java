...
public class GsonUtils {
     ...
     */
    public static <T> void register(Class<T> parent, Class<? extends T>[] children) {
        RuntimeTypeAdapterFactory<T> runtimeTypeAdapterFactory =
                RuntimeTypeAdapterFactory.of(parent, parent.getSimpleName().toLowerCase(Locale.ROOT));

        for (Class<? extends T> child : children) {
            if (!parent.isAssignableFrom(child)) {
                throw new HummerException(parent + " is not assignable from " + child);
            }
            runtimeTypeAdapterFactory.registerSubtype(child, child.getSimpleName().toLowerCase(Locale.ROOT));
        }

        factories.add(runtimeTypeAdapterFactory);
    }
     ...
     *
     * @param json    Json字符串
     * @param typeOfT Json支持转换的泛型类型；
     *                形如：new TypeToken&lt;HashMap&lt;String, Object&gt;&gt;(){}.getType(); <br>
     *                因为Java不支持HashMap&lt;String, Object&gt;.class，所以需要借助TypeToken类获取泛型类型
     * @return 对象
     */
    ...
    private final static class GsonHolder {
        ...
        static {
            ...
            // 注册 RuntimeTypeAdapterFactory
            factories.forEach(gsonBuilder::registerTypeAdapterFactory);

            // 注册 CustomizedObjectTypeAdapter
     ...
     */
    private final static class RuntimeTypeAdapterFactory<T> implements TypeAdapterFactory {
     ...
     */
    private final static class CustomizedObjectTypeAdapter extends TypeAdapter<Object> {
